...
func (o *SSHOptions) Run(f util.Factory) error {
	...
	if !o.Operator {
		newTarget := target.NewTarget(sshTarget.GardenName(), "", *shoot.Spec.SeedName, "")

		seedClient, err := manager.SeedClient(ctx, newTarget)
		if err != nil {
		...
		if o.Operator {
			...
			if len(machines) != len(nodes) {
				...
				for _, machine := range machines {
					...
					if _, ok := nodeSets[machine.Labels["node"]]; !ok {
						nodes = append(nodes, corev1.Node{
							ObjectMeta: metav1.ObjectMeta{Name: machine.Labels["node"]},
